/*
 * Hilly_Condition_sbs.c
 *
 * Automatically generated s-function with I/O interface for:
 * Component: Hilly_Condition
 * Component Simulink Path: Hilly_Condition
 * Simulation Mode: SIL
 *
 */

#define S_FUNCTION_NAME                Hilly_Condition_sbs
...
static RegMdlInfo rtMdlInfo_Hilly_Condition[1] = { "",
  ...
  memUnitSizeBytes)
{
  ...
  pCodeInstrServiceData->infoPath =
    "C:/Adaptive_Front_Light_System_V1.0.0/AFLS_V1.0.0/ASW/SWC/Hilly/Mdl/Hilly_Condition_ert_rtw/sil";
...
static boolean_T startAndSetupApplication(SimStruct *S)
{
...

  {
    ...
    {
      mxArray * codeGenComponent = mxCreateString("Hilly_Condition");
      mxArray *rhs[3];
      char * simulinkBlockPath = getSimulinkBlockPath(S);
      if (simulinkBlockPath == NULL) {
        ssSetErrorStatus(S,
...

                  {
...

                    {
...

                      {
                        ...
                        {
                          copyIOData(simDataMemUnitPtr, mxMemUnitPtr,
                                     &mxMemUnitPtr, num_elements, sizeof(int8_T));
                        }
                      }
                    }
                  }
...
static void mdlInitializeSizes(SimStruct *S)
{
  ...
  slmrInitializeIOPortDataVectors(S, 5, 2);
  ...
  if (S->mdlInfo->genericFcn != (NULL) &&
    ...
    ssRegModelRefMdlInfo(S, "Hilly_Condition", rtMdlInfo_Hilly_Condition, 0);
  }
...
static void XILoutputTID01(SimStruct *S, int tid)
{
...

  {
    ...
    if (IOBufferPtr != NULL) {
      void * pXILService = (void *) ssGetPWorkValue(S, 9);
      if (pXILService != NULL) {
        if (xilHostAppSvcGrowIOBuffer(pXILService, IOBufferPtr, 14, 0)!=
            XILHOSTAPPSVC_SUCCESS) {
          return;
        }                              /* if */
  ...
  {
    {
      ...
      /* dispatch command to the target */
      if (commandDispatch(S, IOBufferPtr, 14)!=XILHOSTAPPSVC_SUCCESS) {
        return;
      }                                /* if */
